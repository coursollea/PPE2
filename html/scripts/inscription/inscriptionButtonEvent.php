<?php include('../../tools/dbFunctions.php');
	...
	if(isset($_GET['inscription']))
	{
		...
		if(count($empties) == 0 && $_GET['checked'] == true)
		{
			...
			{
				$bdd = createPDO();
				$query = 'INSERT INTO Compte(nom, prenom, username, password, email)
							values(?, ?, ?, ?, ?);';
				
				$execute = $bdd->prepare($query);
				$execute->execute([$_GET['name'], $_GET['firstname'], $_GET['username'], $_GET['password'], $_GET['mail']]);
			}
